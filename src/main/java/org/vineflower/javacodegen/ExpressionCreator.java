...
import org.vineflower.javacodegen.expression.*;
...
import org.vineflower.javacodegen.vars.VarState;
...
public class ExpressionCreator {
	...
	public void random(Type type, StringBuilder builder, VarsEntry vars) {
		if (type instanceof PrimitiveTypes primitiveType) {
			...
			if (type == BasicType.STRING) {
				builder.append(createRandomString(random.nextInt(6) + 2).toJava());
	...
	Expression createStandaloneExpression(Type targetType, VarsEntry vars) {
		...
		return builder -> builder.append("System.out.println(").append(createRandomString(3).toJava()).append(")");
	}
